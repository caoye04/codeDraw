...
class NumpyLinearModel():
    ...
    def loss_v(self, v_pred:np.ndarray, v_trgt:np.ndarray) -> float:
        ...
        ########################
        B = v_trgt.shape[0]
        return np.sum((v_pred - v_trgt)**2) / B
    ...
    def grad_v(self, X:np.ndarray, v_trgt:np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        diag_term = 1 - v_pred**2
        diff_term = 2 * (v_pred - v_trgt) / B
        
        grad_w_v = (X.T @ (diag_term * diff_term)).flatten()
        grad_b_v = np.sum(diag_term * diff_term, axis=0)
